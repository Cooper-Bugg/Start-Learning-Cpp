...
- `page-fault-sim` — FIFO and LRU page replacement simulator for any reference string and frame count
- `address-translator` — virtual to physical address translation with hex output and missing page handling
...
- `bakery-buffer` — bounded buffer producer-consumer with mutex and condition variables, clean shutdown
- `deadlock-demo` — intentional deadlock, Coffman conditions, lock ordering fix, monitor thread
- `maze-solver` — recursive backtracking solver that reads a 2D text maze, marks the solution path, and reports steps and cells visited.
  - Goal: recursive backtracking, choose/explore/unchoose pattern, call stack as implicit data structure.
- `sorting-visualizer` — quicksort and mergesort side by side with timing, comparison counts, and optional step-by-step visualization.
  - Goal: algorithmic complexity, instrumented code, O(n log n) behavior on sorted vs random input.
- `network-analyzer-stl-upgrade` — STL container upgrade of network-analyzer-cpp: replace manual structures with unordered_map, set, and queue.
  - Goal: idiomatic STL usage, container selection, before/after diff of the same algorithms.
...
- `sudoku-solver` — recursive backtracking Sudoku solver with constraint checking and recursive call counting.
  - Goal: backtracking applied to constraint satisfaction, isValid() performance, search space awareness.
...
- `foundational-library` — lightweight C++ library of core building blocks: Span, Arena, UniquePtr, Optional, Result, Vector, String, RingBuffer, HashMap.
  - Goal: build STL-level primitives from scratch, pluggable allocator model, no-exceptions design.
- `network-analyzer-cpp` — C++ port of the Java graph analyzer: Dijkstra's, articulation points, manual memory
- `float-inspector-cpp` — CLI IEEE-754 bit inspector and arithmetic step visualizer ported from Java
